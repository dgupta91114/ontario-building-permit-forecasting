...
def run_reproducibility_check(root: Path | None = None) -> dict:
    ...
    required = [
        ...
        base / "outputs" / "official_run_manifest.json",
        base / "data" / "raw" / "download_manifest.json",
    ]
    ...
    if dataset.exists():
        ...
        dataset_checks = {
            ...
            "demo_rows": int(frame.get("source_status", pd.Series(dtype=str)).astype(str).eq("DEMO").sum()),
            "cmas": sorted(frame["cma"].dropna().unique().tolist()),
    ...
    report = {
        "passed": (
            ...
            and dataset_checks.get("duplicate_cma_target_rows", 1) == 0
            and dataset_checks.get("missing_target", 1) == 0
            and dataset_checks.get("demo_rows", 1) == 0
            and len(dataset_checks.get("cmas", [])) == 5
            and dataset_checks.get("target_date_max") == "2026-05-01"
        ),
