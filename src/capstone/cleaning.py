...
def clean_building_permits(
    ...
    if df is None:
        ...
        df = _read_statcan_csv(_manifest_csv(manifest, "product_id", product_id, base))
    ...
    chosen = {
        "seasonal_adjustment": choose_dimension_label(df[seasonal_col], **fcfg["seasonal_adjustment"]),
        "value_type": choose_dimension_label(df[value_type_col], **fcfg["value_type"]),
